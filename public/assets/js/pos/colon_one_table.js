let ColOneTable
let ColOneTotalPrice
let ColOneTotalArticle
$(document).ready(function () {
  ColOneTable = $('#col_1_table').DataTable({
    processing: true,
    paging: false,
    serverSide: true,
    ajax: {
    ...
    columns: [
      ...
      },
      {
        ...
        className: 'text-center pe-0',
        render: function (data, type, row) {
          return ` <a id="${data._id}" data-bs-toggle="tooltip" aria-label="Fshij" data-kt-delete-table-col-1="true"
      ...
      if (document.getElementById('col_1_table_search').value === '') {
        // Calculate total_price and update 'colon_1_article'
        let total_price = 0;
        for (let i = 0; i < data.data.length; i++) {
          total_price += parseFloat(data.data[i].total_price);
        }
        document.querySelector('[col-1-delete="all"]').id = data.sale
        document.querySelector('[col-1-delete-i="all"]').id = data.sale


        ColOneTotalPrice = total_price.toFixed(2)
        ColOneTotalArticle = data.recordsTotal
        document.getElementById('colon_1_article').textContent = data.recordsTotal;
        document.getElementById('price-col-1').textContent = total_price.toFixed(2);
        ...
        $('tbody tr', this).each(function () {
          ...
          qtyInput.on('input', function () {
            ...
            $('#col_1_table').find('tr').each(function (i, e) {
              if (i !== 0) {
            ...
            $('#price-col-1').text(total_price.toFixed(2));
            ColOneTotalPrice = total_price.toFixed(2)
  ...
  const search_input1 = document.getElementById('col_1_table_search')
  search_input1.addEventListener('keyup', (e) => {
    ColOneTable.search(e.target.value).draw()
  })
  const resetSuccess = () => {
    ColOneTable.search(document.getElementById('col_1_table_search').value).draw()
  }
  function swalDelete() {
    const n = document.querySelectorAll(
      '[data-kt-delete-table-col-1="true"]'
    )
    n.forEach((e) => {
...
const searchArticleCol1 = document.getElementById('article-table-search')
searchArticleCol1.addEventListener('input', async (e) => {
  const radioButtons = document.querySelectorAll('[name="radioGroup-col1"]');
  ...
  if (selectedValue === 'name') {
    const nameRegex = new RegExp(searchArticleCol1.value, 'i')
   let filter  = articles.filter(item => nameRegex.test(item.name)).slice(0, parseInt(document.getElementById('article_1_length').value))
    if (filter.length>0) {
  ...
  if (selectedValue === 'code') {
    const nameRegex = new RegExp(searchArticleCol1.value, 'i')
    let filter   = articles.filter(item => nameRegex.test(item.code)).slice(0, parseInt(document.getElementById('article_1_length').value))
    if (filter.length>0) {
      updateDataTable(filter)
    }
    else {
...
searchArticleCol1.addEventListener('keyup',async(e)=>{
  const radioButtons = document.querySelectorAll('[name="radioGroup-col1"]');
  ...
  if ((searchArticleCol1.value.length === 13 && selectedValue === 'barcode')||(selectedValue === 'barcode' && e.keyCode === 13)) {
    const foundArticle = articles.find(item => item.barcode === searchArticleCol1.value);
    if (foundArticle) {
      searchArticleCol1.value = ''
      updateDataTable([foundArticle])
...
$('#qty-check-1').on('change', () => {
  document.getElementById('qty-col1').value = 1
  if (document.getElementById('qty-check-1').checked) {
    document.getElementById('qty-col1').classList.add('d-none')
  }
  else {
    document.getElementById('qty-col1').classList.remove('d-none')
  }
})