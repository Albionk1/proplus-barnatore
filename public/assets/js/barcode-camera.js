var barcode = function () {
	...
	function init() {
		...
		if (navigator.mediaDevices && navigator.mediaDevices.enumerateDevices) {
			navigator.mediaDevices.enumerateDevices()
				.then(function (devices) {
					...
					if (videoDevices.length > 0) {
						// Select the last video device (which is typically the back camera)
						var selectedDevice = videoDevices[videoDevices.length - 1];
						return navigator.mediaDevices.getUserMedia({ video: { deviceId: { exact: selectedDevice.deviceId } } });
					} else {
						// If no video devices are found, just use the default
			...
			// Handle the case where the mediaDevices API is not supported
