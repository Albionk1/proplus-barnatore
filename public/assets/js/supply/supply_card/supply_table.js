...
$(document).ready(function () {
  suppliertable = $('#table_supply').DataTable({
    processing: true,
    serverSide: true,
    ajax: {
      url: '/api/v1/supply/get-supplies-card',
      data: (d) => {
          d.unit = document.getElementById('unit').value,
          d.status = document.getElementById('pay-status').value
          d.supplier = location.pathname.split('/')[3]
      }
    },
    columnDefs: [
    ...
    columns: [
      ...
      },
      {
        ...
        render: function (data, type, row) {
          ...
          <a href="/admin/supplier-see/${data.supplier}"
      ...
      for(let i =0;i<data.data.length;i++){
        if(total_not_paid+data.data[i].price){
          total=parseFloat(total+data.data[i].price)
          if(data.data[i].paid_status===true){
            total_paid=parseFloat(total_paid+data.data[i].price)
          }
          if(data.data[i].paid_status===false){
            total_not_paid=parseFloat(total_not_paid+data.data[i].price)
          }
        }
      }
      document.getElementById('buy-total').textContent = total + ' €';
      document.getElementById('total-paid').textContent = total_paid+ ' €';
      document.getElementById('total-not-paid').textContent = total_not_paid + ' €';
    },lengthMenu: [
      [10, 20, 50, 100],
      [10, 20, 50, 100],
    ]
          ...
          preConfirm: async () => {
            ...
            try {
              ...
              if (data.status === 'success') {
                ...
                suppliertable.search(document.getElementById('search-table').value).draw()
                return true
              }
            }
            catch (err) {
