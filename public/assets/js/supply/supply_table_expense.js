let supplierTableExpense
$(document).ready(function () {
  supplierTableExpense = $('#expense-table').DataTable({
    processing: true,
    serverSide: true,
    ajax: {
      url: '/api/v1/supply/get-supplies-list',
      data: (d) => {
        d.type = 'expense_supply',
          d.unit = document.getElementById('unit').value,
          d.year = document.getElementById('year-expense').value,
          d.status = document.getElementById('status-expense').value
      }
    },
    columnDefs: [
  ...
  var buttons = new $.fn.dataTable.Buttons(supplierTableExpense, {
    buttons: [
      {
        extend: 'excelHtml5',
        title: documentTitle,
  ...
  }).container().appendTo($('#print-expense'));
  document.getElementsByClassName('dt-buttons btn-group flex-wrap')[0].classList.remove('flex-wrap')

  const search_input1 = document.getElementById('search-table_expense')
  search_input1.addEventListener('keyup', (e) => {
    supplierTableExpense.search(e.target.value).draw()
  })
  const resetSuccess = () => {
    supplierTableExpense.search('').draw()
  }
