let supplierTableDamaged
$(document).ready(function () {
  supplierTableDamaged = $('#damaged-table').DataTable({
    processing: true,
    serverSide: true,
    ajax: {
      url: '/api/v1/supply/get-supplies-list',
      data: (d) => {
        d.type = 'damaged_supply',
          d.unit = document.getElementById('unit').value,
          d.year = document.getElementById('year-damaged').value,
          d.status = document.getElementById('status-damaged').value
      }
    },
    columnDefs: [
      ...
      },
      {
        ...
        render: function (data, type, row) {
          ...
          <i
            ...
            data-kt-delete-table-damaged="true"
                 aria-label="Fshij"
  ...
  var buttons = new $.fn.dataTable.Buttons(supplierTableDamaged, {
    buttons: [
      {
        extend: 'excelHtml5',
        title: documentTitle,
  ...
  }).container().appendTo($('#print-damaged'));
  document.getElementsByClassName('dt-buttons btn-group flex-wrap')[0].classList.remove('flex-wrap')

  const search_input1 = document.getElementById('search-table_damaged_search')
  search_input1.addEventListener('keyup', (e) => {
    supplierTableDamaged.search(e.target.value).draw()
  })
  const resetSuccess = () => {
    supplierTableDamaged.search('').draw()
  }
          ...
          preConfirm: async () => {
            ...
            try {
              ...
              if (data.status === 'success') {
                ...
                supplierTableDamaged.search(document.getElementById('search-table_damaged_search').value).draw()
                return true
              }
            }
            catch (err) {
...
$('#status-damaged').change(()=>{
  supplierTableDamaged.search(document.getElementById('search-table_damaged_search').value).draw()
})
$('#unit').change(()=>{
  supplierTableExpense.search(document.getElementById('search-table_expense').value).draw()
  supplierTableDamaged.search(document.getElementById('search-table_damaged_search').value).draw()
