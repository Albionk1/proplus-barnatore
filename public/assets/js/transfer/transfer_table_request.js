...
$(document).ready(function () {
   transfertable = $('#transfer_request').DataTable({
    processing: true,
    serverSide: true,
    ajax:{url:'/api/v1/transfer/get-transfers-requests',
     data:(d)=>{
    ...
    document.getElementById('total_request').textContent=data.requestTotal
  ...
  const search_input1 = document.getElementById('transfer_request_search')
  search_input1.addEventListener('keyup', (e) => {
    transfertable.search(e.target.value).draw()
  })
  const resetSuccess = () =>{
    transfertable.search(document.getElementById('transfer_request_search').value).draw()
  }
  function swalDelete() {
